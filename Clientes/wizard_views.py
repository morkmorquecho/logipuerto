...
from .models import ClientePadre, Documentos, ContactoClientePadre
...
class CrearClientePadreWizardView(GroupRequiredMixin,PasswordResetEmailMixin, MessageMixin,PasswordResetView, SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        with transaction.atomic():
            try:
                ...
                #obtener la instancia del usuario a partir del nombre del usuario proporcionado de RegistrarUserPadreView
                try:
                    username = self.request.session.get('usuario')  # Misma clave que en RegistrarUserPadreView
                ...
                # Extraer los datos ManyToMany antes de crear el objeto
                servicios_data = cliente_data.pop('servicios', None)

                # Crear el objeto ClientePadre (sin los campos ManyToMany)
                cliente_padre = ClientePadre.objects.create(
                    user=usuario,
                    **cliente_data)

                # Asignar las relaciones ManyToMany después de crear el objeto
                if servicios_data:
                    cliente_padre.servicios.set(servicios_data)
                
                
                ContactoClientePadre.objects.create(
                ...
                if 'documentos' in forms_dict and forms_dict['documentos'].cleaned_data:
                    Documentos.objects.create(
                        cliente_padre=cliente_padre,  # Usar la instancia creada
                ...
                self.success("Se ha creado el cliente correctamente")
                return redirect(reverse_lazy('core:inicio'))
            
            except Exception as e:
                self.error(f"Error al crear el cliente: {str(e)}")
                return redirect(reverse_lazy('core:inicio'))
